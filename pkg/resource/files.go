...
func NewFiles(path string, options *FilesOptions) *Files {
	return &Files{
		fsys:    &archives.DeepFS{Root: filepath.Clean(path)},
		options: options,
	}
}
...
func (r *Files) Walk(fn WalkFunc) error {
	return iofs.WalkDir(r.fsys, ".", func(path string, d iofs.DirEntry, err error) error {
		...
		// Let the calling code know we've entered into a sub FS
		if outer, inner := r.fsys.SplitPath(path); len(inner) != 0 {
			path = outer + ":" + inner
		}
