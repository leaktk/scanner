...
// DecodeBase64 returns the data with the b64 values decoded in-place
func DecodeBase64(chunk []byte) []byte {
	encodedSubstrings := getSubstringsOfCharacterSet(chunk, 20, b64CharsetMapping, b64EndChars)
	...
	if len(decodedSubstrings) > 0 {
		...
		result.Grow(len(chunk))

		start := 0
		for _, encoded := range encodedSubstrings {
			if decoded, ok := decodedSubstrings[encoded]; ok {
				end := bytes.Index(chunk[start:], []byte(encoded))
				if end != -1 {
					result.Write(chunk[start : start+end])
					result.Write(decoded)
					start += end + len(encoded)
				}
			}
		}
		result.Write(chunk[start:])
		return result.Bytes()
	}
